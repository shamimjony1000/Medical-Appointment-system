...
def patient():
    ...
    if request.method=="POST":
        ...
        query=db.engine.execute(f"INSERT INTO `booking_req` (`email`,`name`,`gender`,`slot`,`disease`,`time`,`date`,`dept`,`number`) VALUES ('{email}','{name}','{gender}','{slot}','{disease}','{time}','{date}','{dept}','{number}')")
...
@app.route('/booking_request',methods=['POST','GET'])
@login_required
def pending():
    em=current_user.email
    if current_user.usertype=="Doctor":
        query=db.engine.execute(f"SELECT * FROM `booking_req`")
        return render_template('booking_request.html',query=query)
    

@app.route('/pending_bookings',methods=['POST','GET'])
@login_required
def pending_bookings_user():
    em=current_user.email
    if current_user.usertype=="Patient":
        query=db.engine.execute(f"SELECT * FROM `booking_req`")
        return render_template('pending_bookings.html',query=query)
...
@app.route("/deny/<string:rid>",methods=['POST','GET'])
@login_required
def deny(rid,mode='deny'):
    db.engine.execute(f"DELETE FROM `booking_req` WHERE `rid`={rid}")
    if mode=='deny':
        flash("Slot Denied Successful","danger")
    elif mode=='approve':    
        flash("Slot Approved Successful","success")

    return redirect('/booking_request')



@app.route("/approve/<string:rid>",methods=['POST','GET'])
@login_required
def approve(rid):
    query=BookingReq.query.filter_by(rid=rid).first()  
    email=query.email
    name=query.name
    gender=query.gender
    slot=query.slot
    ...
    dept=query.dept
    number=query.number
    insert_query= db.engine.execute(f"INSERT INTO `patients` (`email`,`name`,`gender`,`slot`,`disease`,`time`,`date`,`dept`,`number`) VALUES ('{email}','{name}','{gender}','{slot}','{disease}','{time}','{date}','{dept}','{number}')")    
    deny(rid,mode='approve') 
